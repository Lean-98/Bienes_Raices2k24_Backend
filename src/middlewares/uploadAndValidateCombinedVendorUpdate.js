...
import { vendorsSchema } from '../schemas/vendors.js'

/*
  El middleware `uploadAndValidateCombinedVendorUpdate` gestiona la carga de archivos,
...
export const uploadAndValidateCombinedVendorUpdate = multer({
  ...
  fileFilter: async (req, file, cb) => {
    try {
      ...
      const vendor = await VendorModel.find({ id })

      if (!vendor) {
        throw new ValidationError([
          {
            message: 'Invalid UUID: Vendor ID does not exist in the database',
          },
        ])
      }
      ...
      req.validatedInput = vendorsSchema.parse(inputData)
      cb(null, true)
    } catch (error) {
